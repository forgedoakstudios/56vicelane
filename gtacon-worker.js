...
export default {
  ...
  async fetch(request, env) {
    ...
    // Called by the admin page to force a specific condition
    if (url.pathname === '/gtacon-update' && request.method === 'POST') {
      let body;
      try {
      ...
      const condition = parseInt(body.condition);
      if (!condition || condition < 1 || condition > 5) {
        return new Response(JSON.stringify({ error: 'condition must be 1-5' }), {
          status: 400, headers: { 'Content-Type': 'application/json', ...CORS }
        });
      }

      const conditionNames = { 1:'Maximum', 2:'Critical', 3:'Significant', 4:'Elevated', 5:'Cold' };

      // Get previous state to preserve signals
      ...
      const result = {
        ...
        changed: previousCondition !== condition,
        previousCondition: previousCondition,
        manualOverride: true,
        stat_confidence: body.stat_confidence || (previous ? previous.stat_confidence : '78%'),
        stat_signals: body.stat_signals || (previous ? previous.stat_signals : '16'),
        note: body.note || buildNote(condition, KNOWN_SIGNALS, daysToLaunch),
        signals: previous ? previous.signals : {
          ceoMarketingConfirmed: KNOWN_SIGNALS.ceoMarketingConfirmed,
          psBundleAnnounced: KNOWN_SIGNALS.psBundleAnnounced,
          trailer3Live: KNOWN_SIGNALS.trailer3Live,
          rockstarAdsActive: KNOWN_SIGNALS.rockstarAdsActive,
          leakerActivityHigh: KNOWN_SIGNALS.leakerActivityHigh,
        },
      };
      ...
      console.log(`[GTACON] Manual override: CON ${condition} — ${result.name}`);
      return new Response(JSON.stringify({ ok: true, condition: condition, name: result.name }), {
    ...
    return new Response(JSON.stringify({
      status: 'GTACON Worker Active',
      endpoints: ['/gtacon-status (GET)', '/gtacon-update (POST)', '/gtacon-run (GET)']
    }), {
      status: 200,
      headers: { 'Content-Type': 'application/json', ...CORS }
    });
