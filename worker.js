...
// ============================================================
//  CONTENT ENGINE CORS  (added June 20, 2026)
//  Allows the 56ViceLane admin page (browser) to call this
//  worker's new /engine/* endpoints below.
// ============================================================
const ALLOWED_ORIGINS = [
  'https://56vicelane.com',
  'https://www.56vicelane.com',
  'https://forgedoakstudios.github.io',
  'http://localhost:3000',
];
function engineCorsHeaders(origin) {
...
export default {
  async fetch(req, env) {
    ...
    // ── CONTENT ENGINE: admin page generation proxy ──
    // Preflight for the engine endpoint
    if (url.pathname === '/engine/generate' && req.method === 'OPTIONS') {
      return new Response(null, { status: 204, headers: engineCorsHeaders(origin) });
    }

    if (url.pathname === '/engine/generate' && req.method === 'POST') {
      return handleEngineGenerate(req, env, origin);
    }
    ...
    return Response.json({
      status: 'online',
      version: '5.1',
      endpoints: ['/run', '/drafts', '/debug-all', '/test-claude', '/clear-cache?confirm=yes', '/engine/generate (POST, used by admin page)']
    });
  },
...
// ============================================================
//  CONTENT ENGINE HANDLER
//  Forwards admin-page generation requests to Claude using the
//  same ANTHROPIC_KEY already configured on this worker.
// ============================================================
async function handleEngineGenerate(req, env, origin) {
  const headers = engineCorsHeaders(origin);

  if (!env.ANTHROPIC_KEY) {
    return new Response(
      JSON.stringify({ error: 'Worker is missing ANTHROPIC_KEY. Set it in Cloudflare Dashboard → Workers → Settings → Variables (encrypted). This gets wiped on every wrangler deploy — re-add it after each deploy.' }),
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: 'Upstream request failed: ' + e.message }),
