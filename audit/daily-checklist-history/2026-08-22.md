...
**Generated:** 2026-08-22 (Central Time)
...
- [ ] **n8n's MCP token has now been down 3+ days running (2026-08-19,
      -20, and -21), degrading every breaking- and digest-article run
      in that window.** Every time: image generation failed (generic
      `gta6-hero.png` placeholder used instead of real hero/inline art)
      — the direct HTTP fallback to `n8n.56vicelane.com` is also
      blocked by this sandbox's egress proxy, so there was no working
      path any of those days. On breaking articles this also meant no
      Discord post and no video-SLA alert. Articles still published/
      drafted fine (text/build/dedup-state all unaffected), just
      missing real images + Discord + SLA alert every time. This is a
      clear pattern, not a fluke — worth checking why the n8n MCP
      connection isn't staying authorized between sessions; a longer-
      lived credential or different auth approach is probably needed.
      Full detail: `audit/2026-08-19-news-scan-articles-log.md`,
