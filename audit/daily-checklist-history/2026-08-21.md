...
**Generated:** 2026-08-21 (Central Time)
...
- [ ] **n8n's MCP token expired 3 days running now (2026-08-19, -20, and
      -21), degrading every breaking-article run in that window.** Each
      time: image generation failed (generic `gta6-hero.png` placeholder
      used instead of real hero/inline art), the Discord social-blast
      post never went out, and the video-SLA alert never fired — the
      direct HTTP fallback to `n8n.56vicelane.com` is also blocked by
      this sandbox's egress proxy, so there was no working path any of
      the three days. Articles still published fine (text/build/dedup-
      state all unaffected), just missing images + Discord + SLA alert
      every time. This is now a clear pattern, not a fluke — worth
