...
**Generated:** 2026-08-20 (Central Time)
...
- [ ] **n8n's MCP token expired 2 days running (2026-08-19 and 2026-08-20),
      degrading both days' breaking-article runs.** Each time: image
      generation failed (generic `gta6-hero.png` placeholder used
      instead of real hero/inline art), the Discord social-blast post
      never went out, and the video-SLA alert never fired — the direct
      HTTP fallback to `n8n.56vicelane.com` is also blocked by this
      sandbox's egress proxy, so there was no working path either day.
      Articles still published fine (text/build/dedup-state all
      unaffected), just missing images + Discord + SLA alert both days.
      Worth checking why the n8n MCP connection isn't staying
      authorized between sessions — a longer-lived credential or
      different auth approach may be needed if this keeps recurring
      daily. Full detail: `audit/2026-08-19-news-scan-articles-log.md`,
